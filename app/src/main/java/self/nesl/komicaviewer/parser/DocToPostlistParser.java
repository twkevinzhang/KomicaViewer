...
public class DocToPostlistParser {
    ...
    public DocToPostlistParser(Board board) {
        this.board = board;
    }
    ...
    public ArrayList<Post> tableToPostlist(Document doc) {
    ...
    public ArrayList<Post> jsonToPostlist(JSONArray json_arr) throws JSONException {
    ...
    public Post func2(Element thread) {
        //傳入標準格式(綜合 threads)
        Post post = null;
...
//            Log.e("DP", "post_detail:" + post_detail);
    ...
    public ArrayList<Post> homepageToPostlist(Document doc) {
        ArrayList<Post> postlist = new ArrayList<Post>();

        //get post secret name
        String title=doc.getElementById("fsub").attr("name");
...
//        }
        ...
        for (Element thread : threads.getElementsByClass("thread")) {
            postlist.add(func2(thread));
        }
        return postlist;
    }

    public ArrayList<Post> searchResultToPostlist(Document doc) {
        ArrayList<Post> postlist = new ArrayList<Post>();
