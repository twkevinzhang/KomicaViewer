...
public abstract class BaseViewModel extends ViewModel {
    ...
    public void update(KThread newPost){
        KThread oldPost=post.getValue();
        if(oldPost!=null){
            ArrayList<KThread> arr=oldPost.getReplies(false,0);
            for (KThread subPost:newPost.getReplies(false,0))
                if(!arr.contains(subPost)) arr.add(subPost);
            newPost.setTree(arr);
        }
    ...
    private void post(JSONObject response){
